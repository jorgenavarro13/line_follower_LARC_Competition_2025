...
ModeParameters params_segura = {
  ...
  85,  // Kp
  12,  // Kd
  {-6.9, -6.3, -3.8, 1, 1, 3.8, 6.3 , 6.9}, // weights
  0.75 // curveSlowdown
};
...
ModeParameters params_media = {
  ...
  85,  // Kp
  12,  // Kd
  {-7.2, -6.7, -3.8, 1, 1, 3.8, 6.7 , 7.2}, // weights
  0.75 // curveSlowdown
};
...
ModeParameters params_rapida = {
  ...
  99,  // Kp //85 (original), 
  12,  // Kd //11 (original),
  {-7.9, -6.4, -4.0, 1, 1, 4.0, 6.4 , 7.9}, // weights (usando los de Media) original (7.5 en esquinas y en penultimos en 6.3)
...
void simpleLineFollower(int sensors[8]) {
  ...
  if (blackCount >= 7) {
    ...
    } else if (!allBlackConfirmed && millis() - allBlackStartTime >= 200) { // Usando un valor medio

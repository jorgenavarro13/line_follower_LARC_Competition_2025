...
float baseSpeed = 145;    // velocidad base reducida
float boostSpeed = 155;   // 🔸 velocidad boost reducida
float Kp = 85;
float Kd = 12;
...
// =====================================================
void simpleLineFollower(int sensors[8]) {
  float weights[8] = {-6.7, -7.8, -4.0, -1, 1, 4.0, 7.8 , 6.7};
  int activeCount = 0;
  float weightedSum = 0;
  int blackCount = 0;
  ...
  if (blackCount >= 7) {
    ...
    } else if (!allBlackConfirmed && millis() - allBlackStartTime >= 120) {

...
float baseSpeed = 140;    // velocidad base reducida
float boostSpeed = 150;   // 🔸 velocidad boost reducida
float Kp = 85;
float Kd = 12;
...
// =====================================================
void simpleLineFollower(int sensors[8]) {
  float weights[8] = {-6.8, -6.0, -3.8, 1, 1, 3.8, 6.0 , 6.8};
  int activeCount = 0;
  float weightedSum = 0;
  int blackCount = 0;
  ...
  if (blackCount >= 7) {
    ...
    } else if (!allBlackConfirmed && millis() - allBlackStartTime >= 190) {
